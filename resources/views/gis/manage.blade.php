      ...
      <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-4 col-lg-4 pt-1">
          <div class="procurement-title">
            <h5 class="d-inline">
                <i class="fa fa-file-text-o" aria-hidden="true"></i> GIS
            </h5>                
          </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-8 col-lg-8">
          <a class=" btn sub-btn float-right" href='{{ url("dashboard/gis/create") }}' ><i class="fa fa-plus"> </i> Add  Record </a>
          <a class=" btn sub-btn float-right" href='{{ url("dashboard/gis") }}' ><i class="fa fa-dashboard"> </i> GIS Dashboard </a>
        </div>
      </div>
    </div>
  </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                              <td>{{ $loop->index+$files->firstItem() }} </td>
                                ...
                                <td>
                                   <form class="delete" action="{{ route('gis.delete',$item->id) }}" method="POST">
                                    <input type="hidden" name="_method" value="DELETE">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <a class="btn btn-info btn-sm  fa fa-edit " href="{{ route('gis.edit',$item->id) }}" title="Edit"></a>
