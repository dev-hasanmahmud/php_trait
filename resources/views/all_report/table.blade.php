            ...
            <ul class="pagination">
                <li class="page-item {{ $files->currentPage() == 1 ? ' disabled' : '' }}">
                    <a class="page-link main-pagination"
                        action="{{ $files->url($files->currentPage() - 1) . '&package_id=' . $package_id . $filter_data . $category_id }}"
                ...
                @for ($i = 1; $i <= $files->lastPage(); $i++)
                    <li class="page-item {{ $files->currentPage() == $i ? ' active' : '' }}">
                        {{-- <input type="text" hidden
                            ...
                            id="url_{{ $i }}"> --}}
                        <a class="page-link main-pagination"
                            action="{{ $files->url($i) . '&package_id=' . $package_id . $filter_data . $category_id }}"
                            href="javascript:void(0)" data-id="{{ $i }}">{{ $i }}</a>
                    </li>
                @endfor
